...
from cleanvoice import ApiError, Cleanvoice, FileValidationError
...
def test_process_auto_enables_transcription_for_summarize(cleanvoice_client):
    """summarize should automatically turn on transcription."""
    with patch.object(cleanvoice_client, "_poll_for_completion") as mock_poll, \
        ...
        cleanvoice_client.process("https://example.com/audio.mp3", summarize=True)

        sent_request = mock_create.call_args.args[0]
        assert sent_request.input.config.summarize is True
        assert sent_request.input.config.transcription is True


def test_process_auto_enables_summary_for_social_content(cleanvoice_client):
    """social_content should automatically turn on summarize and transcription."""
    with patch.object(cleanvoice_client, "_poll_for_completion") as mock_poll, \
         patch.object(cleanvoice_client, "_transform_result") as mock_transform, \
         patch.object(cleanvoice_client.api_client, "create_edit") as mock_create:
        ...
        cleanvoice_client.process("https://example.com/audio.mp3", social_content=True)

        sent_request = mock_create.call_args.args[0]
        assert sent_request.input.config.social_content is True
        assert sent_request.input.config.summarize is True
        assert sent_request.input.config.transcription is True
...
def test_process_invalid_config_positive_target_lufs(cleanvoice_client):
    ...
    with pytest.raises(
        FileValidationError, match="target_lufs must be a negative number"
    ):
        cleanvoice_client.process(
            "https://example.com/audio.mp3",
            {"target_lufs": 10},
...
def test_create_edit_api_client_error(cleanvoice_client):
    ...
    with patch.object(cleanvoice_client.api_client, "create_edit") as mock_create:
        ...
        with pytest.raises(ApiError, match="Rate limit exceeded"):
            cleanvoice_client.create_edit(
                "https://example.com/audio.mp3", {"fillers": True}
            )
...
def test_get_edit_unexpected_error(cleanvoice_client):
    ...
    with patch.object(cleanvoice_client.api_client, "retrieve_edit") as mock_retrieve:
        ...
        with pytest.raises(
            ApiError, match="An unknown error occurred while retrieving edit"
        ):
...
def test_poll_for_completion_retrieve_error(cleanvoice_client):
    ...
    with patch.object(cleanvoice_client.api_client, "retrieve_edit") as mock_retrieve:
        mock_retrieve.side_effect = ApiError("Bad request", 400)

        with pytest.raises(ApiError, match="Bad request"):
            cleanvoice_client._poll_for_completion("edit-123")


def test_poll_for_completion_retries_transient_retrieve_error(cleanvoice_client):
    """Transient retrieve errors should not fail polling immediately."""
    success_response = Mock(status="SUCCESS", task_id="task-123")
    success_response.result = Mock()
    ...
    with patch.object(cleanvoice_client.api_client, "retrieve_edit") as mock_retrieve, \
        ...
        response = cleanvoice_client._poll_for_completion("edit-123")
...
def test_process_none_config_handling(cleanvoice_client):
    ...
    with patch.object(cleanvoice_client, "_poll_for_completion") as mock_poll, \
        ...
        cleanvoice_client.process("https://example.com/audio.mp3", None)
...
def test_transform_result_malformed_edit_result(cleanvoice_client):
    ...
    response = Mock()
    response.result = {"invalid": "structure"}
